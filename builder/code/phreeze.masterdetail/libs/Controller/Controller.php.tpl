 ...
 * @version 1.0
 */
class {$configJSON["replaceVars"]["controllerName"]}Controller extends AppBaseController
{
	...
	public function ListView()
	{
	    ...
	    $id = RequestUtil::Get('id');
 		...
 		$pkm = $this->Phreezer->GetPrimaryKeyMap('{$configJSON["masterTable"]}');
        ...
        ${$configJSON["masterTable"]} = $this->Phreezer->Query('{$configJSON["masterTable"]}',$criteria);
        ${$configJSON["masterTable"]} = ${$configJSON["masterTable"]}->ToObjectArray(true,$this->SimpleObjectParams());
        $this->Assign("id",$id); 
        $this->Assign("masterData",${$configJSON["masterTable"]});  
		$this->Render();
	}
